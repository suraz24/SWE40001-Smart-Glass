...
const getCenterPt = pts => pts.reduce(
  ...
  //Finds X-coordinate of Center
const getConvexHullCenterX =(contour)=>{
  //Calculates Hull Points
  ...
  //Calculating x-coordinate of Centre
  var xpt = 0;
...
//Calculates Y coordinate of Center
const getConvexHullCenterY =(contour)=>{
  //Calculates Hull Points
  const hullIndices = contour.convexHullIndices();
  const contourPoints = contour.getPoints();
  const hullPointsWithIdx = hullIndices.map(idx => ({
  ...
  //Calculating y-coordinate of Centre
  var ypt = 0
  for (var i=0;i<hullPoints.length;i++)
  {
...
grabFrames(webcamPort, delay, (frame) => {
  const resizedImg = frame.resizeToMax(640);

  const handMask = makeHandMask(resizedImg);
  const handContour = getHandContour(handMask);
  //commenting out to prevent timeout
  if (!handContour) {
    return;
  }
  ...
  //Finding the x and y coordinates of the center of the Convex Hull
  const centerX=getConvexHullCenterX(handContour);
  const centerY=getConvexHullCenterY(handContour);

  //Testing if functions return values
  ...
  result.drawRectangle(
    ...
    //draws ellipse around center of convex hull
  result.drawEllipse(
    new cv.RotatedRect(new cv.Point(centerX, centerY), new cv.Size(20, 20), 0),
